...
# bash 3pp_extensions_finder_intron_length_100.sh <(grep "gene" /Users⁩/sekaran⁩/ownCloud⁩/conda_xtender⁩/gmap_dir⁩/GMAP_3prime_partial.gff3 | cut -f1,4,5,7,9| awk -F"=" '$4=$1' OFS="\t" | cut -f1,2,3,4,7 | awk '{print $5,$1,$2,$3,$4}' OFS="\t" |  awk 'NR==FNR {a[$1] = $2; next} { $6=a[$1] ;print $0}' OFS="\t" 3pp_ids_strand.txt  - | awk '{print $2,$3,$4,$5,$1,$6}' OFS="\t") > Extensions_3pp_intron_length_200.bed
...
grep $scaffold /Users/sekaran/ownCloud/conda_xtender/gmap_dir/GMAP_3prime_partial.gff3| grep "gene" | grep $transcript | cut -f1,4,5,7,9  | awk '{OFS=FS="\t"}  {if($4 == "+") {print $1,$3-1,$3,$4,$5} else if($4 =="-") {print $1,$2,$2+1,$4,$5}}' - > ./Transcript_Scaffold_bed_files/$transcript.bed
