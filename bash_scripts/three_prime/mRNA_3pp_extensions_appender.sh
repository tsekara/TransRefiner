...
##########  bash 3pp_extension_appender_v2.sh <(sort -u -t$'\t' -k 6,6 $bed_file | cut -f1,5,6,9)

#time_stamp=$(date +%Y-%m-%d-%T)

mkdir -p 3pp_extension_and_original_fasta
    ...
    while IFS= read -r line; do
      ...
      grep "$transcript" $bed_file | awk '{print $1"\t"$2"\t"$3"\t"$6"\t"$4"\t"$5"\t"$7"\t"$8"\t"$9}' | bedtools getfasta -name -fi $genome_fasta -bed - | perl -p -e 's/:.*//g' -  | awk '/^>/ {if(prev!=$0) {prev=$0;printf("\n%s\n",$0);} next;} {printf("%s",$0);} END {printf("\n");}' - | tail -n+2  > ./3pp_extension_and_original_fasta/"$transcript"_extension.fasta

      grep ">" ./3pp_extension_and_original_fasta/"$transcript"_extension.fasta | perl -p -e 's/>//g' - | seqtk subseq -l 100000 $transcriptome_fasta - > ./3pp_extension_and_original_fasta/"$transcript"_original.fasta

      paste ./3pp_extension_and_original_fasta/"$transcript"_extension.fasta ./3pp_extension_and_original_fasta/"$transcript"_original.fasta | awk '{if (NR%2==0) {print $1 $2} else {print $1}}'
      ...
      grep ">" ./3pp_extension_and_original_fasta/"$transcript"_extension.fasta | perl -p -e 's/>//g' - | seqtk subseq $transcriptome_fasta - > ./3pp_extension_and_original_fasta/"$transcript"_original.fasta

      paste ./3pp_extension_and_original_fasta/"$transcript"_original.fasta ./3pp_extension_and_original_fasta/"$transcript"_extension.fasta | awk '{if (NR%2==0) {print $1 $2} else {print $1}}'

      ########################################################################################################################################################################################################
      elif [[ ($genome_orientation == "-") && ($transcript_orientation == "+") ]]
      then

      grep -w "$transcript" $bed_file | awk '{print $1"\t"$2"\t"$3"\t"$6"\t"$4"\t"$5"\t"$7"\t"$8"\t"$9}' | bedtools getfasta -name -fi $genome_fasta -bed - | perl -p -e 's/:.*//g' -  | fastx_reverse_complement -i - | awk '/^>/ {if(prev!=$0) {prev=$0;printf("\n%s\n",$0);} next;} {printf("%s",$0);} END {printf("\n");}' - | tail -n+2  > ./3pp_extension_and_original_fasta/"$transcript"_extension.fasta
      ...
      then
      grep "$transcript" $bed_file | awk '{print $1"\t"$2"\t"$3"\t"$6"\t"$4"\t"$5"\t"$7"\t"$8"\t"$9}' | bedtools getfasta -name -fi $genome_fasta -bed - | perl -p -e 's/:.*//g' - | fastx_reverse_complement -i -  | awk '/^>/ {if(prev!=$0) {prev=$0;printf("\n%s\n",$0);} next;} {printf("%s",$0);} END {printf("\n");}' - | tail -n+2  > ./3pp_extension_and_original_fasta/"$transcript"_extension.fasta
