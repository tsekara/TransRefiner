...
##########  bash 5pp_extensions_appender_v3.sh <(sort -u -t$'\t' -k 6,6 only_5pp_internal_extensions.bed | cut -f1,5,6,9)       
...
grep -w "$transcript" only_5pp_internal_extensions.bed | awk '{print $1"\t"$2"\t"$3"\t"$6"\t"$4"\t"$5"\t"$7"\t"$8"\t"$9}' | bedtools getfasta -name -fi ~/Transcriptome_Enhancement/SmedAsxl_genome_v1.1.fa -bed - | sed 's/:.*//' -  | awk '/^>/ {if(prev!=$0) {prev=$0;printf("\n%s\n",$0);} next;} {printf("%s",$0);} END {printf("\n");}' - | sed '/^\s*$/d' -> ./5pp_extension_and_original_fasta_$time_stamp/"$transcript"_extension.fasta

grep ">" ./5pp_extension_and_original_fasta_$time_stamp/"$transcript"_extension.fasta | sed 's/>//' - | seqtk subseq -l 100000 ~/ownCloud/dd_Smed_v6.pcf.contigs.fasta - > ./5pp_extension_and_original_fasta_$time_stamp/"$transcript"_original.fasta
