...
##########  bash 3pp_extension_appender_v2.sh <(sort -u -t$'\t' -k 6,6 Cleaned_3pp_internal_extensions_int_len_200.bed | cut -f1,5,6,9)       
...
grep "$transcript" Cleaned_3pp_internal_extensions_int_len_200.bed | awk '{print $1"\t"$2"\t"$3"\t"$6"\t"$4"\t"$5"\t"$7"\t"$8"\t"$9}' | bedtools getfasta -name -fi ~/Transcriptome_Enhancement/SmedAsxl_genome_v1.1.fa -bed - | sed 's/:.*//' -  | awk '/^>/ {if(prev!=$0) {prev=$0;printf("\n%s\n",$0);} next;} {printf("%s",$0);} END {printf("\n");}' - | sed '/^\s*$/d' -> ./3pp_extension_and_original_fasta_$time_stamp/"$transcript"_extension.fasta

grep ">" ./3pp_extension_and_original_fasta_$time_stamp/"$transcript"_extension.fasta | sed 's/>//' - | seqtk subseq -l 100000 ~/ownCloud/dd_Smed_v6.pcf.contigs.fasta - > ./3pp_extension_and_original_fasta_$time_stamp/"$transcript"_original.fasta
...
grep ">" ./3pp_extension_and_original_fasta_$time_stamp/"$transcript"_extension.fasta | sed 's/>//' - | seqtk subseq ~/ownCloud/dd_Smed_v6.pcf.contigs.fasta - > ./3pp_extension_and_original_fasta_$time_stamp/"$transcript"_original.fasta
...
grep -w "$transcript" Cleaned_3pp_internal_extensions_int_len_200.bed | awk '{print $1"\t"$2"\t"$3"\t"$6"\t"$4"\t"$5"\t"$7"\t"$8"\t"$9}' | bedtools getfasta -name -fi ~/Transcriptome_Enhancement/SmedAsxl_genome_v1.1.fa -bed - | sed 's/:.*//' -  | fastx_reverse_complement -i - | awk '/^>/ {if(prev!=$0) {prev=$0;printf("\n%s\n",$0);} next;} {printf("%s",$0);} END {printf("\n");}' - | sed '/^\s*$/d' - > ./3pp_extension_and_original_fasta_$time_stamp/"$transcript"_extension.fasta
...
then
grep "$transcript" Cleaned_3pp_internal_extensions_int_len_200.bed | awk '{print $1"\t"$2"\t"$3"\t"$6"\t"$4"\t"$5"\t"$7"\t"$8"\t"$9}' | bedtools getfasta -name -fi ~/Transcriptome_Enhancement/SmedAsxl_genome_v1.1.fa -bed - | sed 's/:.*//' - | fastx_reverse_complement -i -  | awk '/^>/ {if(prev!=$0) {prev=$0;printf("\n%s\n",$0);} next;} {printf("%s",$0);} END {printf("\n");}' - | sed '/^\s*$/d' -> ./3pp_extension_and_original_fasta_$time_stamp/"$transcript"_extension.fasta
