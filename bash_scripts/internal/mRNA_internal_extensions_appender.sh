...
#paste ./internal_extension_and_original_fasta/"$transcript"_5pp_extension.fasta ./internal_extension_and_original_fasta/"$transcript"_original.fasta ./internal_extension_and_original_fasta/"$transcript"_3pp_extension.fasta

######################################################################################################################################################################################################
elif [[ ($genome_orientation == "-") && ($transcript_orientation == "+") ]]
then

grep -w "$transcript"  $bed_5pp_file | awk '{print $1"\t"$2"\t"$3"\t"$6"\t"$4"\t"$5"\t"$7"\t"$8"\t"$9}' | bedtools getfasta -name -fi $genome_fasta -bed - | perl -p -e 's/:.*//g' -  | awk '/^>/ {if(prev!=$0) {prev=$0;printf("\n%s\n",$0);} next;} {printf("%s",$0);} END {printf("\n");}' - | tail -n+2  | fastx_reverse_complement -i - > ./internal_extension_and_original_fasta/"$transcript"_5pp_extension.fasta

grep -w "$transcript" $bed_3pp_file | awk '{print $1"\t"$2"\t"$3"\t"$6"\t"$4"\t"$5"\t"$7"\t"$8"\t"$9}' | bedtools getfasta -name -fi $genome_fasta -bed - | perl -p -e 's/:.*//g' -   | awk '/^>/ {if(prev!=$0) {prev=$0;printf("\n%s\n",$0);} next;} {printf("%s",$0);} END {printf("\n");}' - | tail -n+2 | fastx_reverse_complement -i - > ./internal_extension_and_original_fasta/"$transcript"_3pp_extension.fasta
