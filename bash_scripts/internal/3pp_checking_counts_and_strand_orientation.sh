...
#The input for this script is the output of "3pp_extensions_finder.sh".
...
#echo -e "$line" > temp.txt --------> Prints the current line into a file called "temp.txt" in the working directory.

#plus=$(awk '$4=="+"' Sorted_"$scaffold".bed | bedmap --count temp.txt - ) ---------------> Extracts all the reads in "+" orientation in that particular "scaffold" and later counts the number of reads within in the range specifed in temp.txt and stores in a varibale called "plus"


#minus=$(awk '$4=="-"' Sorted_"$scaffold".bed | bedmap --count temp.txt  -) ---------------> Extracts all the reads in "-" orientation in that particular "scaffold" and later counts the number of reads within in the range specifed in temp.txt and stores in a varibale called "minus"

#paste <(echo -e $line) <(echo $plus) <(echo $minus) -------------> Prints the current line alogn with plus and minus in a spcae delim format


#awk -v OFS="\t" '$1=$1' -  --------------------> As the above command generates the output in non tab delim format, the sed command replaces spcaes with tab delim format
