...
#extended_df=as.data.frame(setDT(extended)[, .SD[which.max(pct_query_len)], by=X.query_acc])  
...
Extn.cov_equals_than_original.cov= ifelse((orig_extn_with_evalue_target_len$pct_target_len.y == 
                                              orig_extn_with_evalue_target_len$pct_target_len.x), T, F),
Extn.cov_less_than_original.cov= ifelse((orig_extn_with_evalue_target_len$pct_target_len.y < 
                                             orig_extn_with_evalue_target_len$pct_target_len.x), T, F),
...
Extn.cov_equals_Orig.cov=orig_extn_with_evalue_target_len[grep("TRUE",orig_extn_with_evalue_target_len$Extn.cov_equals_than_original.cov),]
number_of_uniq_trans_Extn.cov_equalto_Orig.cov= length(unique(Extn.cov_equals_Orig.cov$X.query_acc))

Extn.cov_less_Orig.cov=orig_extn_with_evalue_target_len[grep("TRUE",orig_extn_with_evalue_target_len$Extn.cov_less_than_original.cov),]
number_of_uniq_trans_Extn.cov_less_than_Orig.cov= length(unique(Extn.cov_less_Orig.cov$X.query_acc))
...
message("Extn.cov < Orig.cov:",number_of_uniq_trans_Extn.cov_less_than_Orig.cov)
