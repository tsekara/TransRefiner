setwd("~/XXX")
library( DESeq2 )
RPF<-read.table("xx.txt", fill=T,header=T, sep="\t")
head(RPF)
MRNA<-read.table("yy.txt", fill=T,header=T, sep="\t")
head(MRNA)
RPF_MRNA<-merge(RPF, MRNA, by.x="id", by.y="id", all=T)
...
mar.default <- c(5,4,4,2) + 0.1
par(mar = mar.default + c(0,1, 0, 0))
plot(data.sub2$log2FoldChangeR~data.sub2$log2FoldChangeM,  family="sans", font.axis=2, cex.axis=1.4, cex.lab=1.6, font.lab=2, xlab="log2 foldchange mRNA", ylab="log2 foldchange RPF", pch=19, cex=.7, xlim=c(-2.5, 2.5), ylim=c(-2.5,2.5), col = ifelse(data.sub2$padjM< .1 & data.sub2$padjR< .1, "red3", ifelse(data.sub2$padjM< .1, "coral", ifelse(data.sub2$padjR< .1, "blue", "gray50" ) ) ))
rect (-3, -3, 3, 3, col = alpha("grey", 0.2))

### points(data.df$log2FoldChangeM.y[orthologs], data.df$log2FoldChangeR.x[orthologs], pch=18, cex=2.2, col="black")
