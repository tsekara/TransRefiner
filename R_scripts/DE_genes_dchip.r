...
col_size=0.3,row_size=0.3,genes=rownames(data),p_classes = colnames(data), c_margins = c(6,6))
{
  ...
  else
  {
    ...
    setwd("D:/output")
    pdf(file=paste("dchip.clustering_DE_genes.pdf",sep=""))
...
writetable=function(object,filename,p_append=FALSE,p_sep="\t",p_row=T,p_col=T)
{
  write.table(object,file= paste(filename, ".txt", sep = ""),append=p_append,quote=FALSE,sep=p_sep,row.names=p_row,col.names=p_col)
}
       setwd("D:/output")
 dchip.clustering(data.quantiles[DEgenes_A_vs_B,],annotation,main="",col_size=1,p_classes=samplesheet$"Replicate")

 return("dchip.clustering_DE_genes.pdf")
 }